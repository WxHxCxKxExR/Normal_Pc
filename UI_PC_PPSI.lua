local library = {
	...
	colored = {},
	configuration = {
		hideKeybind = Enum.KeyCode.F1,
		smoothDragging = false,
		easingStyle = Enum.EasingStyle.Quart,
		easingDirection = Enum.EasingDirection.Out
	},
	colors = {
		main = Color3.fromHex('FF1B1B'),--a6f4f0
		background = Color3.fromHex('171717'),
		outerBorder = Color3.fromHex('141414'),
		innerBorder = Color3.fromHex('3E3E3E'),
		topGradient = Color3.fromHex('0A0A0A'),
		bottomGradient = Color3.fromHex('0A0A0A'),
		sectionBackground = Color3.fromHex('191919'),
		section = Color3.fromHex('D8D8D8'),
		otherElementText = Color3.fromHex('9F9F9F'),
		elementText = Color3.fromHex('848484'),
		elementBorder = Color3.fromHex('141414'),
		selectedOption = Color3.fromHex('292929'),
		unselectedOption = Color3.fromHex('282828'),
		hoveredOptionTop = Color3.fromHex('323232'),
				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Ui")
							if not fold then
								makefolder("./Ui")
							end
							fold = isfolder("./Ui/Themes")
							if not fold then
								makefolder("./Ui/Themes")
							end
							fold = isfolder("./Ui/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Ui Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Ui/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Ui/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Ui/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Ui" then
		library.WorkspaceName = convertfilename(windowName, "Ui")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Ui") then
						makefolder("./Ui")
					end
					local common_string = "./Ui/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Ui/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			Name = "Image Asset ID",
			Placeholder = "rbxassetid://5553946656", --  11995210995
